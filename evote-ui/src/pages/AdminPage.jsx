...
export default function AdminPage() {
  ...
  async function attach() {
    ...
    try {
      ...
      if (!ok)
        throw new Error(
          "No contract code at this address on current RPC."
        );
  ...
  async function endElectionNow() {
    ...
    try {
      ...
      if (
        mgmt.admin &&
        your.toLowerCase() !== mgmt.admin.toLowerCase()
      ) {
      ...
      try {
        ...
        setMgmtMsg(
          "endNow() not available; trying setEnd(now) / setWindow(…, now) …"
        );
      }
      ...
      try {
        ...
        // setEnd might not exist; fall through
      }
      ...
      try {
        ...
        // setWindow might not exist either; fall through
      }
...
`}
          ...
          {deployedAddr && (
            <div className="kv mt8">
              <b>Contract:</b>{" "}
              <span className="mono">{deployedAddr}</span>
            </div>
          )}
        </section>
      ) : (
          ...
          {mgmt.contract && (
            <>
              ...
              <div className="kv">
                <b>Start:</b>{" "}
                {mgmt.startTs
                  ? new Date(mgmt.startTs * 1000).toLocaleString()
                  : "—"}
              </div>
              <div className="kv">
                <b>End:</b>{" "}
                {mgmt.endTs
                  ? new Date(mgmt.endTs * 1000).toLocaleString()
                  : "—"}
              </div>
              <div className="kv">
                <b>Admin? </b>
